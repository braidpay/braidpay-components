...
export default function PaymentForm({
  ...
  // Show error message if status has an error
  useEffect(() => {
    if (status.error) {
      setValidationError(status.error);
      // If there's an error with the email, reset the form
      if (status.error.includes('email')) {
  ...
  return (
    <div className={className}>
      ...
      <Dialog open={isPaymentModalOpen} onOpenChange={handlePaymentModalChange}>
        <DialogContent 
          className="max-w-4xl w-full !h-[90vh] p-0 bg-background"
          aria-describedby="payment-form-dialog-description"
        >
          <div id="payment-form-dialog-description" className="sr-only">
            BraidPay payment window for {productName}. You can complete your payment using USDC or other stablecoins.
          </div>
